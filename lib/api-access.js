 ...
 *    @property {Number} codeExpiresInSecs: This expiration time for the code that is returned by the api access login
 *    @property {Function} register: this function is when the register route is called and received the request body 
 *                                   as a paramater.
 *                                   the function must return an object representing the user.
 *                                   Later on the user object will used to calculate the payload and token
 *                                   that will allow the application client to connect the zerv api
 *
