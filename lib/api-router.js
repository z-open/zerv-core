...
function apiRouter(io, event) {
    ...
    function onEvent(socket) {
        socket.on(event, function (call, serializedJson, fn) {
            ...
            if (!_.isNil(serializedJson)) {
                try {
                    const t = serializedJson.data;// new Uint8Array(serializedJson);
                    data = jsonbin.deserialize(t);
                } catch (err) {
    ...
    function Handler(socket, func) {
        ...
        function emit(event, data) {
            const serializedJson = jsonbin.serialize(data);
