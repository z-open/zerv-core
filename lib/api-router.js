...
function ApiRouter(io, event, transport, defineTransaction) {
    const routes = {};
    ...
    function Handler(socket, func, options) {
        ...
        function getTransaction() {
            if (!trans) {
                trans = defineTransaction('NEW', null, {
                    name: 'Api Router',
                    user: this.user
                });
            }
        ...
        function execute(params) {
            ...
            if (options.transactional) {
                return getTransaction().execute(() => thisHandler.exe(params));
            }
            // any result is wrapped in a promise if it is not already
            return Promise.resolve(() => thisHandler.exe(params));
        }
    };
};
