...
function socketAuthorize(options, io, coreModule, onConnection) {
    ...
    userSessionService.init(coreModule, io, options.tokenExpiresInMins);
    ...
    return function(socket) {
        ...
        socket.on('disconnect', function(err) {
            ...
            userSessionService.onUserDisconnect(socket, err);
        });
        // @ts-ignore
    ...
    async function authorizeWithAuthCodeAndRefreshedToken(decodedToken, onSuccess, onError, context) {
        ...
        if (await blackListService.checkIfTokenRefreshedInSharedDb(context.data.token)) {
    ...
    function initNewConnection(decodedToken, context) {
        ...
        if (currentSocket.tenantId || !options.getTenantId) {
            ...
            userSessionService.onUserConnect(currentSocket);
            return {
                catch: _.noop
            };
        }
