...
function socketAuthorize(options, io, coreModule, onConnection) {
  ...
  function onNewSocket(socket) {
    const authTimeout = setAuthorizationTimeout(socket, options);
  ...
  async function authenticateSocket(socket, data) {
    try {
      ...
      if (!socket.userId) {
        return await initNewConnection(socket, connData);
      }
      ...
      return await maintainConnection(socket, connData);
    } catch (error) {
  ...
  /**
     ...
     */
  async function initNewConnection(currentSocket, connData) {
    try {
      ...
      if (connData.decodedToken.jti >= 1) {
        if (!await userSessionService.isUserSessionActive(connData.origin)) {
          userSessionService.logout(connData.origin, 'inactive_session_timeout_or_session_not_found');
  ...
  /**
     ...
     * @param {*} connData
     */
  async function maintainConnection(currentSocket, connData) {
    try {
      ...
      const [newToken, payload] = refreshToken(decodedToken, connData.origin, currentSocket.tenantId);
      ...
      emitToken(currentSocket, newToken, oldToken, decodedToken.exp);
    } catch (error) {
