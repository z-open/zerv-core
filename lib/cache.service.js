...
async function cacheData(key, value, options = {}) {
    if (options.expirationInMins) {
        await service.getRedisClient().setex(formatKeyName(key, options.prefix), options.expirationInMins * 60, value);
    } else {
        await service.getRedisClient().set(formatKeyName(key, options.prefix), value);
    }
}
