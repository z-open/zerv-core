...
function getRedisClient() {
    ...
    if (!redisClient) {
        redisClient = service._createIoRedis();
    }
    return redisClient;
}

function _createIoRedis() {
    ...
    // @ts-ignore
    const redisClient = new IoRedis(connectionParams);
    redisClient.on('error', onError);
    return redisClient;
}

function _clearIoRedisInstance() {
    redisClient = null;
}
...
class LocalCache {
    ...
    scanStream(options) {
        // this is a simple mock implementation of redis scanStream which only supports the match option with *
        const match = options.match.substr(0, options.match.length - 1);
        const keys = _.keys(this.data);
        return new Readable({
            objectMode: true,
            read() {
                this.push(_.filter(keys, (key) => key.startsWith(match)));
                this.push(null);
            }
        });
    }
