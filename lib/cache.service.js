...
async function getCachedObjectsWithKeyNameBeginning(keyNameBeginningToMatch, options = {}) {
    ...
    const keys = await getCachedKeys(keyNameBeginningToMatch);
    return getCachedObjects(keys);
}

async function getCachedKeys(keyNameBeginningToMatch, options = {}) {
    if (!isClusterCacheEnabled()) {
        throw new Error('Not implemented in local caching, only redis');
    }
    return new Promise((resolve, reject) => {
        let keys = [];

        // scanStream seems not supported by redis cluster... it could be an issue later on.
        const stream = getRedisClient().scanStream({
        ...
        stream.on('end', () => {
            if (options.prefix) {
                keys = _.map(keys, (key) => key.substring(options.prefix.length));
            }
            resolve(_.uniq(keys));
        });
