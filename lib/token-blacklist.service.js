...
async function isTokenRevoked(token) {
    if (redisService.isRedisEnabled()) {
        const result = await redisService.getRedisClient().get(REDIS_REVOKED_TOKEN_PREFIX + token);
        return result === 'true';
    }
...
async function revokeToken(token) {
    ...
    if (redisService.isRedisEnabled()) {
        ...
        await redisService.getRedisClient().setex(REDIS_REVOKED_TOKEN_PREFIX + token, tokenDisposalInMins * 60, true);
    } else {
