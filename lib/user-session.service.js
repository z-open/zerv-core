...
function init(coreModule, io, maxInactiveTimeForInactiveSession) {
    ...
    service._scheduleUserSessionMaintenance(maxInactiveTimeForInactiveSession);
...
function _scheduleUserSessionMaintenance(maxInactiveTimeForInactiveSession) {
    maxInactiveTimeForInactiveSession = 0.5;
    _clearOldUserSessionsIntervalHandle = setInterval(
        () => {
            service._removeAllInactiveLocalUserSessions(maxInactiveTimeForInactiveSession);
        },
 ...
 */
function _removeAllInactiveLocalUserSessions(maxInactiveTimeForInactiveSession) {
    _.forEach(service.getLocalUserSessions(), (userSession) => {
        if (!userSession.active && moment.duration(moment(new Date()).diff(userSession.lastUpdate)).asMinutes()>maxInactiveTimeForInactiveSession) {
            ...
            service._removeLocalUserSession(userSession, 'garbage_collected');
        }
    });
}
...
function _removeLocalUserSession(userSession, reason) {
...
async function _logoutLocally(userSession, reason) {
    ...
    service._removeLocalUserSession(userSession, reason);
