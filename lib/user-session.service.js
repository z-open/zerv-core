...
const DEFAULT_MAXIMUM_ACTIVE_SESSION_TIMEOUT_IN_MINS = 90 * 24 * 60; // about 3 months
 ...
 * @param {Number} maxInactiveTimeInMinsForInactiveSession
 */
function init(coreModule, io, maxInactiveTimeInMinsForInactiveSession) {
    assert(maxInactiveTimeInMinsForInactiveSession, 'maxInactiveTimeInMinsForInactiveSession must be defined to determine when inactive user session should be released from memory.');
    ...
    service._scheduleUserSessionMaintenance(maxInactiveTimeInMinsForInactiveSession);
...
async function connectUser(socket) {
    return await updateLocalUserSession(socket);
}
...
function _scheduleUserSessionMaintenance(maxInactiveTimeInMinsForInactiveSession) {
    // maxInactiveTimeInMinsForInactiveSession = 0.5;
    _clearOldUserSessionsIntervalHandle = setInterval(
        () => {
            service._removeAllInactiveLocalUserSessions(maxInactiveTimeInMinsForInactiveSession);
        },
        maxInactiveTimeInMinsForInactiveSession * 60000
    );
}
/**
 ...
 */
function _removeAllInactiveLocalUserSessions(maxInactiveTimeInMinsForInactiveSession) {
    _.forEach(service.getLocalUserSessions(), (userSession) => {
        if (!userSession.active && moment.duration(moment(new Date()).diff(userSession.lastUpdate)).asMinutes()>maxInactiveTimeInMinsForInactiveSession) {
...
function _scheduleAutoLogout(userSession) {
    const remainingTimeInMins = userSession. getRemainingTimeInSecs() / 60;
    ...
    logger.info('%s is set to expire in %s min(s) on %s', userSession, remainingTimeInMins.toFixed(1), moment(maximumSessionTime));
...
async function _logoutLocally(userSession, reason) {
    ...
    _.forEach(socketServer.sockets.sockets, async (socket) => {
        if (socket.origin === userSession.origin) {
            await tokenBlacklistService.revokeToken(socket.token);
...
function getTenantMaximumActiveSessionTimeoutInMins(tenantId) {
    ...
    const max = process.env.ZERV_MAX_ACTIVE_SESSION_TIMEOUT_IN_MINS || DEFAULT_MAXIMUM_ACTIVE_SESSION_TIMEOUT_IN_MINS;
    if (_.isNil(value) || value < 1 || value > max || value > DEFAULT_MAXIMUM_ACTIVE_SESSION_TIMEOUT_IN_MINS) {
        return max;
    }
    return value;
}
...
async function saveClusterUserSession(clusterUserSession) {
    await cacheService.cacheData(clusterUserSession.origin, clusterUserSession, {prefix: REDIS_SESSION_PREFIX, expirationInMins: clusterUserSession.maxActiveDuration});
}
