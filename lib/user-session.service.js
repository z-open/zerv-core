...
async function getClusterUserSession(localUserSession) {
    ...
    if (!clusterUserSession) {
        clusterUserSession = {
            ...
            maxActiveDuration: await getTenantMaximumActiveSessionTimeout(localUserSession.tenantId) // in second
        };
...
function scheduleAutoLogout(userSession) {
    const {remainingTime, maximumSessionTime} = calculateRemainingSessionTime(userSession);
    if (remainingTime <= 0) {
...
function calculateRemainingSessionTime(userSession) {
...
function getTenantMaximumActiveSessionTimeout(tenantId) {
    // in minutes
    return 1;
}
