...
async function _logoutLocally(userSession, reason) {
    ...
    _.forEach(socketServer.sockets.sockets, async (socket) => {
        if (socket.origin === userSession.origin) {
            await tokenBlacklistService.blacklistToken(socket.token);
...
async function findClusterUserSession(origin) {
    const result = await redisService.getRedisClient().get(REDIS_SESSION_PREFIX + origin);
    return _.isNil(result) ? null : JSON.parse(result);
}

async function saveClusterUserSession(clusterUserSession) {
    await redisService.getRedisClient().setex(REDIS_SESSION_PREFIX + clusterUserSession.origin, clusterUserSession.maxActiveDuration * 60, JSON.stringify(clusterUserSession));
}

async function deleteClusterUserSession(origin) {
    await redisService.getRedisClient().del(REDIS_SESSION_PREFIX + origin);
}
