...
async function isUserSessionActive(origin) {
    ...
    const lastActivity = Date.now() - new Date(existingSession.lastActivity).getTime();
    const result = lastActivity < (service.getTenantMaximumInactiveSessionTimeoutInMins(existingSession.tenantId) * 60 * 1000);
    return result;
}
...
async function notifyUserSessionActivity(origin, logMsg) {
    ...
    if (localUserSession) {
        localUserSession.lastActivity = new Date();
        localUserSession.currentActivity = logMsg;
    ...
    if (clusterUserSession) {
        clusterUserSession.lastActivity = new Date();
        clusterUserSession.currentActivity = logMsg;
...
async function saveClusterUserSession(clusterUserSession) {
    // wrong this would extend on each save
    logger.warn('WRONG - SECURITY HOLE');
    const expirationInMins = clusterUserSession.maxActiveDuration; //
    await cacheService.cacheData(clusterUserSession.origin, clusterUserSession, {prefix: REDIS_SESSION_PREFIX, expirationInMins});
}
