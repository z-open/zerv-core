...
let localUserSessionDisconnectedListeners = {};
...
const service = {
    ...
    onLocalUserSessionDisconnected,
    ...
    _notifyLocalUserSessionDisconnected
};
...
function init(coreModule, io, inactiveLocalUserSessionTimeoutInMins) {
    ...
    localUserSessionDisconnectedListeners = {};
...
function onLocalUserSessionDisconnected(callback) {
    const listenerId = UUID.v4();
    localUserSessionDisconnectedListeners[listenerId] = callback;
    return () => delete localUserSessionDisconnectedListeners[listenerId];
}
...
function _notifyLocalUserSessionDisconnected(localUserSession, reason, socket) {
    _.forEach(localUserSessionDisconnectedListeners, (listener) => listener(localUserSession, socket.id, reason, socket));
}
...
async function disconnectUser(socket, why) {
    ...
    const localUserSession = await service._updateLocalUserSession(socket);
    _notifyLocalUserSessionDisconnected(localUserSession, why, socket);
}
