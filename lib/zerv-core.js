...
function getSessionEvents(io, onUserConnect, onUserDisconnect) {
    ...
    return {
        // maybe we can check if the user was recently connected (caching session)
        // so it was just a temporary connection lost.. and then maintain the initial session time instead of loosing it each time there is a refresh
