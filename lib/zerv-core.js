...
function getSessionEvents(io, onUserConnect, onUserDisconnect) {
    ...
    return {
        onUserConnect: (socket) => {
            ...
            locallUserSessions[session.origin] = session;
            session.active = true;
...
function isLocalUserSession(userSession) {
    return !_.some(locallUserSessions, existingSession => existingSession.id === userSession.id);
}
...
function countLocalSessionsByUserId(userId) {
    return _.filter(locallUserSessions, existingSession => existingSession.active && existingSession.userId === userId).length;
}
...
function getLocalUserSessions() {
    return _.values(locallUserSessions);
}

function formatSession(socket) {
    let session = locallUserSessions[socket.origin];
